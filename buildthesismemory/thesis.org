...
Special thanks to Victor Pascual and Miquel Oliver for their mentorship. Thanks to Victor Oncins and Angel Elena (craem) for their feedback and help.\\
...
** Basic real-time quality parameters
...
The bandwidth is the number of bits[fn:10] that can arrive from the source to the destination through the network in one unit of time (for example, MB/s). A greater bandwidth delivers higher definition (better quality) of the media content. A packet can be lost if it is never received at the destination; it is represented by packet loss[fn:109] percentage (%). A communication suffers PDV (Packet Delay Variation, \cite{rfc3393}) when different packets arrive with different delays at its destination[fn:9]. It is defined delay as the time between transmitting the packet and arriving at its destination; it is represented by milliseconds (ms). Due to the nature of packet switching, packets can be delivered out of order. The arrived packet can present a corruption of data. The prior parameters presented affect negatively to the communication. Its undesirable effects might be silent lapses and interruptions of the communication.

Real-time communications require low average and guaranteed measure of bandwidth and delay. This two requirements can be achieved easily with the technologies nowadays. For example bandwidth was 56 Kbps in the 90s with dial-up Internet access, and now it is of 24.3 Mbps of download, and 11.2 Mbps of upload according to netindex.com[fn:107][fn:108]. A G.711 call[fn:106] could represent 64 Kbps, which means that it represented 114% of bandwidth usage in the 90s and less than 0.01% now.

# This could be extended with additional quality parameters (see diary.org)

Real-time data transport is managed with UDP. This protocol does not perform retransmission, it means that the communication should be fixed for the next upcoming packets. 
...
This section presents different technologies, some of them have a strong relation with WebRTC: DNS records and pointers, NAT, STUN, TURN, ICE, SDP, HTTP, Websocket, firewall, proxy, LDAP, OAuth.
...
- Strengths
  ...
  - It helps to solve connectivity problems caused by NAT, firewall, etc. (see section [[Internet's evolution]]).
  - It selects particular video and audio codecs to standardize.
  ...
  - The communication between peers is bidirectional and it can be P2P
...
- Threats
  - WebRTC standard does not specify signaling: this can produce a positive or negative fragmentation of projects. Positive fragmentation: different projects for different applications. Negative fragmentation: divided effort.
  - It is a work in progress technology, it is being changed.
...
- Product Owner: /is responsible for maximizing the value of the product and the work of the Development Team/. Product Owner communicates with customers and stakeholders.
...
In order to adapt the different concepts that comprise the scrum methodology for this particular project, the different scrum roles are assumed by the author: Product Owner, Development Team and Scrum Master. This means that the author has to control the project with different points of view that originally corresponds to different people; but with this manner, the author has more autoevaluation tools.

Stakeholders and customers: mentors, University, people interested in the project. The author is interested in the output of the project because is a volunteer in Guifi.net. Mentors participate in this project through regular meetings with the author to discuss the contents. Author sends questions and content to stakeholders in order to obtain feedback.
...
Guifi.net is the selected scenario to design and implement an RTC system.
...
#+CAPTION: Map of Guifi.net's coverage (Catalonia's zone)
#+NAME: fig:guifimap
[[../img/guifi-map.png]]
...
The designed use cases are those that are bold.
...
- Database (DB) is used for authentication only if users are directly registered to service. Also, DB stores and encrypts personal information or preferences for a particular user of web application if succeed in authentication.
...
- Web Application. WebRTC is being implemented in web browsers. That is why is required to use web technologies. Given web application's constraint, other selections have to be done. Another possibility is to do an standalone softphone WebRTC application using the native code[fn:81], but it requires more development and is less flexible (see SWOT analysis).
...
*Reliability of network architecture*. Server part could have a single point of failure that can be mitigated as any common Internet architecture such as DNS or HTTP service: load balancing and/or adding redundancies. In fail's case, components can be easily restored because WebRTC relies in wide known and tested technologies.
** Implementation

Figure [[fig:ai]] shows a software application's selection for each network architecture design component.
...
- APP: jitsi[fn:99]. It is a standalone softphone application which supports multiple operating systems such as Windows, Mac OS X and GNU/Linux.
...
How use cases and network architecture design are implemented in the demo.

Use cases 1 and 2 (see section [[Generic use cases]]) are working as expected. Use case 11 was not included in the demo.

Figure [[fig:ad]] shows network architecture design of the demo. Some of the components are missing from the original network architecture design (see section [[Network architecture design]]).
...
- DB is used with AUTH during SIG. DB for user preferences is not used by WAPP, that part is not implemented.
...
*** Performance
The performance could not be measured appropiately[fn:113], but some of the measures indicate that it is enough for a small community. With organization and investment, the community could grow.
...
Figure [[fig:cdj]] gives samples of jitter (PDV) and delay parameters. Sometimes the network become unstable, in that case was not perceived loss of audio.
...
The suggested direction is firstly to improve currently implemented use cases, secondly to continue to implement those remaining and thirdly to perform a stress test of the demo.
...
To perform a stress test of the demo. This could help to know how much concurrent users allow the network design, and measures of overall latency and bandwidth usage.
...
* Demo installation
Guide to install the demo. It assumes a Debian 7 Wheezy installation with Internet access and administration rights. Demo infrastructure of server part was tested in a common physical machine and in a virtualized one with KVM[fn:111] through Proxmox[fn:112] virtualization management solution.
...
[fn:107] http://www.netindex.com/ (July 2, 2015)

[fn:106] More information about G.711 (see section [[Network requirements]])

[fn:108] This download and upload values can be reached with wireless technologies (see section [[Guifi.net architecture]])

[fn:109] Packet loss can appear in bad wireless links or in congested networks.

[fn:110] Most of them are prior to WebRTC

[fn:111] Kernel-based Virtual Machine, linux kernel virtualization solution

[fn:112] https://www.proxmox.com/

[fn:113] That is why a stress test is added as future work.

[fn:9] The variation in packet delay is sometimes called "jitter"

[fn:114] Application Programming Interface (API) provides an easy way to access internal or external software components and functionalities.
