...
Requirements to send, receive calls, subscribe, unsubscribe and have integration:
- Application interface: gives appropriate interaction to the actors in order to perform the different operations.
- Authentication service: restrict access of service only to permitted users. Differentiate available operations depending on if is user or admin.
- Signalling protocol: manage the side to side connections and logic to establish the call.
- Transport protocol between users and between user and server.
- Database: store and encrypt personal information or preferences for a particular user.
- Connectivity solver: a set of tools to avoid common communication problems that appear in networking scenarios
