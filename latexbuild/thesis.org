...
/Guifi.net uses different protocols/
...
# include network architecture feature heterogeneity.
...
- WebRTC facilitates the use of RTC in organizations. /Small/ organizations can solve the majority of its use cases, large organizations can do it better. Natural evolution of past technologies *FILL technologies?*. Previous attempts were more difficult in general, that is why only it was in large organizations.
- It is required a minimal organization to apply RTC. Because of general architecture design of WebRTC requires at least a centralized server. Centralized server has low load and inexpensive cost. The organization required is similar to a web page project. Therefore, the architecture design cannot be completely decentralized.
- Currently is not possible to use WebRTC and be anonymous, ICE procedure reveals a lot of information about a web page visitor.
- Currently is not supported WebRTC media channel traffic through an HTTP Proxy, but there is a work in progress.
- WebRTC and SIP interoperability: /JsSIP uses the WebRTC stack of the browser (currently just Chrome has a working WebRTC stack), and WebRTC is not "just "RTP" but it requires SRTP (Secure RTP), ICE, a new SDP profile (SAVPF) and so on. It's not possible to make a direct media session between JsSIP (or any other WebRTC based SIP device) and Linphone since Linphone is a basic SIP client with just common plain RTP support, no ICE no SRTP no SAVPF./
...
** TODO Future Work
- work in the remaining use cases.
  - Streaming.
  - Server intercommunication, federation to Guifi.net (inside) and outside (Internet).
- Improve integration of signaling services XMPP[fn:66] and/or SIP[fn:65] inside OpenWRT-based firmwares such as qMp and/or libremesh.
- Develop a WebRTC plugin to Cloudy.
- Develop a web-based CUSAX client (Combined Use of the Session Initiation Protocol (SIP) and the Extensible Messaging and Presence Protocol (XMPP), \cite{rfc7081}). Recommended libraries compatible with websocket: jssip for SIP and strophe.js[fn:67] for XMPP. /It is as easy as 2 iframes/. Take advantages of ease of use of SIP for real-time calls and XMPP list of contacts (roster) for its presence.
