...
Nodes: routers, PC's, /wireless antennas/, ...



*Relationships*

architecture feature associated: centralized, distributed
...
*Redundancy* -> redundancy links

*Scalability*

/comentar pollos de rutas/
...
# tema Cloudy y las ventajas que trae (incluye el dnsservices)
Cloudy[fn:64]
...
** TODO Conclusions
Coming back to the objectives of this project, they are all reachable. WebRTC is open source. Encryption is mandatory for user data. WebRTC is backward compatible to some degree with previous RTC solutions. The provided network architecture design is a good starting point to continue implementing RTC in guifi.net, because the WebRTC design is as scalable and extensible as a web project. The ease of installation and use is being improved by many people, this work tried to summarize some of the work lines and proposed a direction for guifi.net's developer community. For other organizations such as open source communities, companies, etc. a similar approach could be applied.

WebRTC facilitates the use of RTC in organizations. /Small/ organizations can solve the majority of its use cases, large organizations can do it better. WebRTC is a natural evolution of previous technologies. Previous attempts were more difficult in general, that is why only it was /only/ deployed in large organizations.

It is required a minimal organization to apply RTC. Because the general architecture design of WebRTC requires at least one centralized server. Centralized server has low load and inexpensive cost. The organization required is similar to a web page project. Therefore, the architecture design cannot be completely decentralized.

There are three featured topics of WebRTC that are negative. Currently is not possible to use WebRTC and be anonymous, ICE procedure reveals a lot of information about a web page visitor. Currently is not supported WebRTC media channel traffic through an HTTP Proxy; this issue affects guifi.net's traditional model. WebRTC and SIP interoperability needs a gateway for the media channel. The gateway basically requires network resources and associated architecture, but it has low CPU load.
...
** TODO Future Work
It is suggested to improve the current worked use cases and continue with the remaining.

Integrate the proposed network architecture of WebRTC in Cloudy and guifi.net web. Thanks to this two project, ease of installation could arrive to the final user with relatively short development. 

#  - Streaming.
#  - Server intercommunication, federation to Guifi.net (inside) and outside (Internet).

Other interesting related projects to research on:
- Improve integration of signaling services XMPP[fn:66] and/or SIP[fn:65] inside OpenWRT-based firmwares such as qMp and/or libremesh. An approach such that, could decentralize more the RTC system, but requires more intelligence or automatic ways of federation for lots of devices.
- Develop a web-based CUSAX client (Combined Use of the SIP and the XMPP), \cite{rfc7081}). Recommended libraries compatible with websocket: jssip for SIP and strophe.js[fn:67] for XMPP. Take advantages of ease of use of SIP for real-time calls and XMPP list of contacts (roster) for its presence.
...
[fn:72] single administrative entity with a specific routing policy which anounces a network prefix (a set of IPs)
