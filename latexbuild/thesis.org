...
Inside *practice*, there is components' testing. Configuration and execution of network architecture design proposed.
...
- Web Application. WebRTC is being implemented in web browsers. That is why is required to use web technologies. Given web application's constraint, other selections have to be done. Another possibility is to do an standalone WebRTC application, but it requires more development and is less flexible (see SWOT analysis).
  - WAPP: HTML/CSS/JS. HTML (HyperText Markup Langauge) for web content, CSS (Cascading Style Sheets) for web design, JS (Javascript) for language programming features. JS has the commands to access WebRTC API's features.
  - WSRV: HTTP server. It serves HTML/CSS/JS pages to web browsers.
- AUTH: OAuth. It is used to authorize users to use an HTTP service. 
- SIG: SIP
- DB: SQL (poca información, muy estructurada).
- CS: STUN, TURN
- SGW: SIP over Websockets
- MGW: RTP to RTP/SAVPF converter.
# victor pascual dice que normalmente van sin cifrar por las dificultades
# Yes, it’s true that the majority of RTP traffic in VoIP networks is not secured today – in fact, this is one of the very first features customers usually ask vendors to remove in order to meet their budgets. However, when secured, most of the deployments I’ve seen are using SDES (which as mentioned has a strong dependence on the signaling plane security).
# src https://webrtchacks.com/webrtc-must-implement-dtls-srtp-but-must-not-implement-sdes/


- Part1. conceptual selection, example: SIP as signaling service
...
src https://www.terena.org/activities/tf-webrtc/meeting2/slides/20150509-Cisco-WebRTC.pdf
...
- WAPP: tryit.jssip.net[fn:80] , WSRV: apache HTTP server
- AUTH: kamailio
- SIG: kamailio
- DB: SQL (poca información, muy estructurada)
- CS: STUN, TURN [not tested]
- SGW: jssip
- MGW: rtpengine (formerly . src [not tested] http://www.kamailio.org/w/tag/rtpproxy-ng/ (antes rtpproxy-ng el módulo y mediaproxy la pplicación)

# image of kamailio ecosystem
