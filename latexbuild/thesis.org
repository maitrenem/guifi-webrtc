...
** Requirements
*** Generic use cases
Actors or Roles are User and Admin. A call refers to a audio or video call. The *basic service* is defined as it follows:
1. *A user receive a call only if is connected to the service with at least one device.*
...
2. *A user call another user with an audio channel. Optional channels of communication if available: video and chat.*
   - Access to the service through an application.
   - Authenticate.
   - Select the user it is wanted to call.
   - The other user has to accept it to continue.
   - Bidirectional communication.
   - One of the two users stop the communication.

The *advanced service* is defined as it follows:

1. Call history: a user can see the calls sent, received, missed. He can delete it.
...
7. *Integration: all the services are integrated and the account is the same.*
*** Guifi.net use cases service
