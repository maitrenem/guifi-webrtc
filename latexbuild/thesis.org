...
The Internet is in constant evolution, and different mechanisms are necessary for solving problems affecting real-time communications.
...
Firewalls commonly admit traffic via ports 80 and 443. Port 80 is associated with unencrypted web navigation using HTTP[fn:12] (HyperText Transfer Protocol, \cite{rfc2616}). Port 443 is associated with encrypted web navigation using HTTPS (HTTP Secure, \cite{rfc2818}). HTTP and HTTPS, through a web browser, are the most common forms for communicating over the Internet. Both follow a request-response communication model, but a call requires a bidirectional resource. The Websocket protocol \cite{rfc6455} introduces bidirectional communication between browsers via ports 80 and 443. This allows websocket-based services to be accepted by a firewall, and also prepares the way for a new generation of applications which operate between browsers, such as real-time communications.

# - LDAP ?

** TODO Voice, video calls and instant messaging
...
SIP (Session Initiation Protocol, \cite{rfc3261}) is a protocol used to establish, modify and terminate multimedia sessions in the Internet. It is one of the VoIP[fn:14] available services, an alternative to H.323 from the ITU. It is text-based, on a request/response transaction model from HTTP and SMTP; for example encrypted SIP is SIPS, and it has similarities to the relation between HTTP and HTTPS. SIP itself provides a TCP or UDP signalling channel, SDP to agree a media session and a RTP media channel. SIMPLE[fn:16] is a concluded workgroup at IETF that extended SIP with messaging and presence, this source offers an overview of the work done \cite{rfc6914}. Important telecommunication enterprises provides SIP trunking[fn:20] such as Telefonica[fn:17] and AT&T[fn:18].

# http://tools.ietf.org/id/draft-rosenberg-sipping-siptrunk-00.txt

XMPP (Extensible Messaging and Presence Protocol, \cite{rfc6120}), is a protocol to manage instant messaging and presence of your contact list (roster). It is one of the IM (Instant Messaging) applications available. It encrypts its traffic with TLS such as SIP and HTTP. It is text-based streaming XML (Extensible Markup Language) data in close to real-time. XMPP itself provides a signalling channel, typically via the TCP. Jingle \cite{xep0166} is a signalling protocol that extends XMPP with audio and video calls and is designed to interwork with SIP through gateways. Massively used services provides XMPP-based solutions such as WhatsApp Messenger with 800 million monthly active users[fn:19].

missing to put: It is defined at the IETF, and the extensions are worked with the XMPP Foundation with XEV
...
clients problem connecting to this services (firewall). SOlution for SIP: SIP over websockets \cite{rfc7118}. Solution for XMPP. It used BOSH (Bidirectional-streams Over Synchronous HTTP), later websockets. XMPP over websockets \cite{rfc7395}
...
- Media Management
  - MCU
  - SFU
- Datachannel: SCPT over DTLS over UDP
...
[fn:20] SIP trunking means interconnection to this operator through SIP
