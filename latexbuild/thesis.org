...
- MGW: Conversion between SIP and WebRTC media channels is required due to incompatibilities at transport layer. According to an author[fn:86] the majority of RTP traffic in VoIP networks is not secured today; when it is secured, most of the deployments he has seen use SDES (Session Description Protocol (SDP) Security Descriptions for Media Streams \cite{rfc4568}). Guifi.net's VoIP project use RTP tunneled with VPN.
...
- SIG: kamailio[fn:88]. It is a long career project that comes from the origins of SIP Proxy.
- SGW: jssip. It is written by the authors of SIP over websocket RFC.
- MGW: rtpengine[fn:89] (formerly rtpproxy-ng kamailio's module for mediaproxy-ng). It is prepared to interconnect WebRTC to RTP[fn:90]; because it is compatible with SDES, DTLS-SRTP, etc.
- WAPP: tryit.jssip.net[fn:80]. It is a functional demo application proposed by jssip.
  - Browsers: Firefox and Chrome. They are the most used browsers[fn:93].
- WSRV: apache HTTP server. It is a common, easy to use and well-known HTTP server.
...
Not tested (summary): coturn (CS), OAuth (AUTH).
...
WebRTC is still under development. That is why is recommended to use the browsers' latest versions. For Google Chrome, version 42.0.2311.152. For Firefox, Version 38.0.5

# references to appendices

# Deployment (PoC)

Figure [[fig:cc]] shows tryit web client. It is generic, and can be used to login other proxy servers. The login parameters needed are the SIP URI, for example =bob@10.1.56.201=, its password and web socket URI, for example =ws://10.1.56.201:8080=. This means that a kamailio sip proxy has IP =10.1.56.201=, it is listenining websocket port =8080= and it has a user registered called bob.

#+CAPTION: Web SIP Client
#+NAME: fig:cc
[[../img/call-client.png]]

Figure [[fig:cw]] shows the next screen. bob is succesfully registered, that is, he can call or wait a call. To perform a call, fill green box with SIP URI and click call.
...
# websocket web port 80, http proxy traverse signalling but not media

A wireshark[fn:94] capture (figure [[fig:ww]]) shows how it looks a typical call's SIP procedure with websocket. The websocket seven messages are:
1. REGISTER
2. SIP 200 OK
3. INVITE sip:alice@10.1.56.201 (contains SDP offer)
4. SIP 100 TRYING
5. SIP 180 RINGING
6. SIP 200 OK (contains SDP answer)
7. ACK
...
* Install Kamailio with Websocket support
* Install tryit jssip Web SIP Client
